...
(function () {
  ...
  function createChatWidget() {
    ...
    widget.id = "wf-chat";
    widget.innerHTML = `
      <!-- 最小化時のミニボタン（アイコン＋さらに消すボタン） -->
      <div id="wf-chat-mini-wrap" class="wf-chat-mini-wrap" hidden>
        <button id="wf-chat-mini" class="wf-chat-mini" aria-label="チャットを表示">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
          </svg>
        </button>
        <button id="wf-chat-hide-all" class="wf-chat-hide-all" aria-label="完全に隠す">
  ...
  function bindChatEvents() {
    ...
    const miniWrap = document.getElementById("wf-chat-mini-wrap");
    const miniBtn = document.getElementById("wf-chat-mini");
    const hideAllBtn = document.getElementById("wf-chat-hide-all");
    ...
    // 完全に隠すボタン（ミニアイコンの×）→ 完全非表示
    hideAllBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      miniWrap.hidden = true;
      const widget = document.getElementById("wf-chat");
